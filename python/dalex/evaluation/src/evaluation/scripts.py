...
from config import DATA_DIR, DATASETS, METHODS, MODELS
...
def run_explanation_experiments(
    ...
    if method not in METHODS:
        ...
        explainer = dx.Explainer(my_model, X_train, y_train, predict_function=lambda m, d: m.predict(d))

    result_columns = list(X_train.columns) + ["run_idx", "model", "method", "dataset", "observation_id", "n_samples"]
    ...
    for run_idx in range(0, n_runs):
        ...
        if method == "unbiased":
            start = time.time()
            explain = explainer.predict_parts(X_test.iloc[observation_id:observation_id+1], \
                                              type="unbiased_kernel_shap", n_samples=n_samples)
    ...
    if save:
        shap_results.to_parquet(DATA_DIR / "estimates" / f"model_{model}_meathod_{method}_dataset_{dataset}_nruns_{n_runs}_observation_id_{observation_id}_nsmaples_{n_samples}.parquet")
...
@click.option("--observation_ids", type=str, required=True, help="comma separated values pls")
@click.option("--n_samples", type=str, required=False, help="comma separated values pls")
