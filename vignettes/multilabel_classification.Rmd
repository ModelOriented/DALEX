...
```{r}
ve_p <- variable_effect(explain_HR_ranger, variables = "salary", type = "partial_dependency")
plot(ve_p)
```
...
```{r}
ve_a <- variable_effect(explain_HR_ranger, variables = "salary", type = "accumulated_dependency")
plot(ve_a)
```
...
# model_performance and residuals_distribution
...
explain_HR_ranger_residual <- explain(model_HR_ranger,
                              ...
                              colorize = FALSE)
plot(model_performance(explain_HR_ranger_residual))
```
...
```{r}
rd_all <- residuals_distribution(explain_HR_ranger_residual, HR[1,])
plot(rd_all)
