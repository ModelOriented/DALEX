...
yhat.randomForest <- function(X.model, newdata, ...) {
  if (X.model$type == "classification") {
    ...
    if (ncol(pred) == 2 & !is.null(attr(X.model, "predict_function_column"))) return(pred[,attr(X.model, "predict_function_column")])
    else if (ncol(pred) == 2 & is.null(attr(X.model, "predict_function_column"))) return(pred[,2])
...
yhat.svm <- function(X.model, newdata, ...) {
  if (X.model$type == 0) {
    ...
    if (ncol(pred) == 2 & !is.null(attr(X.model, "predict_function_column"))) { # binary classification
...
yhat.cv.glmnet <- function(X.model, newdata, ...) {
  ...
  if (!is.null(X.model$glmnet.fit$classnames)) {
    ...
    if (ncol(pred) == 2 & !is.null(attr(X.model, "predict_function_column"))) {
...
yhat.model_fit <- function(X.model, newdata, ...) {
  if (X.model$spec$mode == "classification") {
    ...
    if (ncol(response) == 2  & !is.null(attr(X.model, "predict_function_column"))) {
