...
test_that("Checks for y",{
  ...
  explainer_ranger_3 <- explain(model_classif_ranger, data = titanic_imputed)
  explainer_ranger_4 <- explain(model_classif_ranger, data = titanic_imputed, y = data.frame(titanic_imputed$survived))
  explainer_ranger_5 <- explain(model_classif_ranger, data = titanic_imputed, y = c(1, 1, 1, 1, 1))
  explainer_ranger_6 <- explain(model_classif_ranger, data = titanic_imputed, y = as.factor(titanic_imputed$survived))
  explainer_ranger_7 <- explain(model_classif_ranger, data = titanic_imputed[,-8], y = titanic_imputed$survived)
...
test_that("Checks for model_info, weights and prints",{
  ...
  explainer_lm_2 <- explain(model_regr_lm, data = apartments, y = apartments$m2.price, weights = rep(1, times = nrow(apartments)))
  explainer_lm_3 <- explain(model_regr_lm, data = apartments, y = apartments$m2.price,
                            weights = data.frame(rep(1, times = nrow(apartments))))
  explainer_ranger_8 <- explain(model_regr_ranger, data = apartments[,-1], y = apartments$m2.price, weights = c(1,1,1,1))
  ...
  expect_is(explain(list(1))$model_info, "model_info")
...
test_that("predict and residual functions", {
  ...
  explainer_ranger_9 <- explain(model_multiclassif_ranger, data = HR, y = HR$status, predict_function = p_fun_ranger)
  explainer_ranger_10 <- explain(model_regr_ranger, data = apartments[,-1], y = as.data.frame(apartments$m2.price), predict_function = p_fun_ranger,
                                ...
                                }, weights =  rep(1, times = nrow(apartments)))
  explainer_ranger_11 <- explain(model_classif_ranger, data = titanic_imputed[,-8], y = titanic_imputed$survived)
  explainer_ranger_12 <- explain(model_multiclassif_ranger, data = HR, y = HR$status)
...
test_that("update_* work", {
  ...
  expect_error(update_label(explainer_lm0, 1))
  expect_error(update_label(list(1), "Label"))
  expect_error(update_data(explainer_lm0, 1))
  expect_error(update_data(list(1), apartmentsTest))
  ...
  expect_is(update_data(explainer_lm0, apartments, as.factor(apartments$m2.price)), "explainer")
})
